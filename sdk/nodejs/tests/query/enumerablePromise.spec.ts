...
import { EnumerablePromise } from "../../query/enumerablePromise";

describe("EnumerablePromise", () => {
    describe("range", () => {
        it("produces an empty array for overlapping ranges", async () => {
            let xs = await EnumerablePromise.range(0, 0).toArray();
            assert.deepEqual(xs, []);

            xs = await EnumerablePromise.range(0, -1).toArray();
            assert.deepEqual(xs, []);
        });

        it("produces an array of one for boundary case", async () => {
            const xs = await EnumerablePromise.range(0, 1).toArray();
            assert.deepEqual(xs, [0]);
        });

        it("can produce a range including negative numbers", async () => {
            const xs = await EnumerablePromise.range(-3, 2).toArray();
    ...
    describe("toArray", () => {
        it("returns empty array for empty enumerable", async () => {
            const xs = await EnumerablePromise.from([]).toArray();
            assert.deepEqual(xs, []);
        });
    });
});
