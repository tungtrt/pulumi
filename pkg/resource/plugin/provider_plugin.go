...
type provider struct {
	...
	enableSecrets bool                             // true if this provider plugin supports secret properties
}
...
func (p *provider) Configure(inputs resource.PropertyMap) error {
	...
	for k, v := range inputs {
		...
		switch {
		case v.IsComputed():
			p.cfgknown, p.enableSecrets = false, false
			close(p.cfgdone)
			return nil
		case v.IsString():
	...
	go func() {
		resp, err := p.clientRaw.Configure(p.ctx.Request(), &pulumirpc.ConfigureRequest{Variables: config})
		if err != nil {
		...
		p.cfgknown, p.enableSecrets, p.cfgerr = true, resp.GetEnableSecrets(), err
		close(p.cfgdone)
	}()
